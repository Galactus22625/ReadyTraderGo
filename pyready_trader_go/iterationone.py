...
#arbitrage only
#we need to add market making as well
#adjust so that market making runs on whatever, use remaining lot space for arbitrage when available


#steps
    #add exit strategy to positinos taken from arbitrage
    #read paper, optimize?
    #create monye maker algo
            #how to optimize? more trades the better
    #combine
    #ensure things arent breached
        #event loop to track things?
        #trades per second
        #how many lots to arbitrage how many to money market making?
    #weight depends on arbitrage?
    #normally, attempt to hold nothing
    #if arbitrage hold max?
...
class AutoTrader(BaseAutoTrader):
    ...
    def __init__(self, loop: asyncio.AbstractEventLoop, team_name: str, secret: str):
        ...
        self.differencetracker = []         #in case we want to only use x recent data points for normal model instead of all data ever
    ...
    def on_order_book_update_message(self, instrument: int, sequence_number: int, ask_prices: List[int],
       ...
       #self.logger.info("received order book for instrument %d with sequence number %d", instrument,
        ...
        if len(self.bids) +len(self.asks)>9:
            self.send_cancel_order(min(min(self.bids, default=math.inf), min(self.asks, default = math.inf)))
    ...
    def on_trade_ticks_message(self, instrument: int, sequence_number: int, ask_prices: List[int],
        ...
        if self.numdatapoints > 30:
            ...
            #exiting at neutral ish, not necessary, only good for  over the same line, but might go over other line so counter productive even
            # elif self.etfprice - self.futureprice > self.differencemean - 0.2 * self.differencestdeviation and self.etfprice - self.futureprice < self.differencemean + 0.2 *self.differencestdeviation and self.activevolume < 190:
            #     if self.position> 0:
            #         self.ask_id = next(self.order_ids)
            #         self.send_insert_order(self.ask_id, Side.SELL, self.etfaskprice - TICK_SIZE_IN_CENTS, self.position, Lifespan.GOOD_FOR_DAY)
            #         self.activesell += self.position
            #         self.activevolume += self.position
            #         self.asks.add(self.ask_id)
            #     elif self.position < 0:
            #         self.bid_id = next(self.order_ids)
            #         #self.bid_price = new_bid_price
            #         self.send_insert_order(self.bid_id, Side.BUY, self.etfbidprice + TICK_SIZE_IN_CENTS, self.position, Lifespan.GOOD_FOR_DAY)
            #         self.activevolume += 10
            #         self.activebuy += 10
            #         self.bids.add(self.bid_id)
