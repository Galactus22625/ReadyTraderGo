...
class AutoTrader(BaseAutoTrader):
    ...
    def on_order_book_update_message(self, instrument: int, sequence_number: int, ask_prices: List[int],
        ...
        if instrument == Instrument.ETF:
            ...
            if self.emergencyhedgecounter > 220:
                if self.hedgeposition < -self.position:
                    bidid = next(self.order_ids)
                    self.send_hedge_order(bidid, Side.BID, MAX_ASK_NEAREST_TICK, -self.position - self.hedgeposition -9)
                    self.hedgebids.add(bidid)
                else:
                    askid = next(self.order_ids)
                    self.send_hedge_order(askid, Side.ASK, MIN_BID_NEAREST_TICK, self.hedgeposition + self.position -9)
                    self.hedgeasks.add(askid)
                self.emergencyhedgecounter = 0
            elif abs(self.hedgeposition + self.position)<10:
                self.emergencyhedgecounter = 0
            # elif self.hedgeposition != -self.position:
            #     if self.hedgeposition < -self.position:
            #         bidid = next(self.order_ids)
            #         self.send_hedge_order(bidid, Side.BID, MAX_ASK_NEAREST_TICK, min(-self.position - self.hedgeposition, 3))
            #         self.hedgebids.add(bidid)
            #     else:
            #         askid = next(self.order_ids)
            #         self.send_hedge_order(askid, Side.ASK, MIN_BID_NEAREST_TICK, min(self.hedgeposition + self.position, 3))
            #         self.hedgeasks.add(askid)
